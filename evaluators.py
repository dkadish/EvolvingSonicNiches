...
class BaseEvaluator:
    ...
    def evaluate(self, genomes, config):
        # self.process = Process(target=self.evaluator, args=(genomes, config)) #FIXME Does this need to be a separate process?
        # self.process.start()
...
class EncoderEvaluator(BaseEvaluator):
    ...
    def evaluator(self, genomes, config):
        ...
        messages = [[[int(random.random() > 0.5) for i in range(3)] for j in range(N_MESSAGES)] for k in range(len(genomes))]
