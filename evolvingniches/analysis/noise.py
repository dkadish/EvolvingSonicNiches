...
from evolvingniches.analysis.data import load_subset_fn
from evolvingniches.visualize import plot_spectrum, plot_stats, plot_species, plot_summary_stats
...
def calculage_usage(spectrum, generations=50, level=0.15):
    ...
    for i in range(total_generations):
        usage_map.append(
            np.sum(extended[i:i + generations, :], axis=0) > level * np.sum(extended[i:i + generations, :]))
    ...
    for i, ind in enumerate(messages):
        ...
        if args.individual and i < number:
            plot_spectrum(usage_map, view=True, title='Usage map (#{})'.format(i),
                          filename='channel_usage_{}.svg'.format(i))
...
def plot_fitness_from_multiple_runs(args):
    ...
    for population in (encoders, decoders):
        ...
        plot_summary_stats(avg, std, best, title='Fitness averages over N runs', xlabel='Generation', ylabel='Fitness',
                           view=True, filename='')
...
def plot_histogram_of_channel_volume(args):
    ...
    print(
        'P-value')  # This is the important one? Confidence that the null hypothesis can (low) or cannot (high) be rejected
