...
from evolvingniches.dataframe import shrink_individuals
from evolvingniches.genome import DefaultGenome
...
def diversity(species: pd.DataFrame, conf_encoders, conf_decoders, individuals: pd.DataFrame = None,
              individuals_file: str = None):
...
def species_summary(individuals: pd.DataFrame = None, individuals_file: str = None, save='species.parquet',
                    config_path=None):
    assert not (individuals is None and individuals_file is None)
